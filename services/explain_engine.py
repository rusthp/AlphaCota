...
def generate_portfolio_explanation(
    ...
    for a in allocations:
        ...
        if momentum < 30.0:
            reason.append(
                "PENALTY DE -20% APLICADO: Empresa sofre ruidosa queda de tendências (Faca caindo) embora tenha salvado o cutoff por fundamentos fortes."
            )
        else:
        ...
        if altman > 2.99:
            reason.append(
                "Altman Z-Score blindado [Zona Segura] (Praticamente zerado o risco de insolvência ou quebra corporativa)."
            )
        elif altman > 1.8:
        ...
        reason.append(
            f"Otimização algorítmica forçou o peso alocado até {weight*100:.2f}% do PL para não estourar a dominância da classe '{asset_class}'."
        )

        selection_logic.append(
            {"ticker": ticker, "classe": asset_class, "weight_pct": round(weight * 100, 2), "reason": reason}
        )
    ...
    return {
        ...
        "fire_projection": {
            ...
            "fire_status": (
                "[GREEN] Independencia em rota garantida."
                if not isinstance(years_to_fire, str)
                else "[RED] Déficit absoluto nos juros de retirada atual requer aumento de salário/aportes."
            ),
        },
    }
