...
def run_allocation_pipeline(
    ...
    for asset in elite_universe:
        ...
        if ticker in allocation:
            ...
            selected_assets_structured.append(
                {"ticker": ticker, "classe": asset["classe"], "quantidade": max(1, int(qtd)), "preco_atual": preco}
            )
    ...
    if current_portfolio is not None:
        ...
        rebalance_flag = detect_universe_change(old_tickers, new_tickers) or should_rebalance(drift, threshold=0.05)
