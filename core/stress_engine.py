...
def apply_stress_scenario(
    ...
    if scenario_key not in STRESS_SCENARIOS:
        raise KeyError(f"Cenário '{scenario_key}' não encontrado. " f"Opções: {list(STRESS_SCENARIOS.keys())}")
...
def format_stress_report(result: dict) -> str:
    ...
    for a in result["assets"]:
        lines.append(
            f"  {a['ticker']:<10} ({a['sector']:<14}) " f"  {a['impacto_%']:>+7.2f}%  R$ {a['impacto_R$']:>10,.2f}"
        )
...
def summarize_stress_suite(suite_results: list[dict]) -> dict:
    ...
    return {
        ...
        "ranking": [
            {"scenario": r["scenario_name"], "drawdown": r["drawdown"], "div_delta": r["dividendos_delta"]}
