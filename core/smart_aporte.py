...
def generateAporteSuggestion(
    classe_prioritaria: str,
    portfolio_ativo: list[dict],
    asset_universe: list[dict],
    valor_aporte: float
) -> dict:

    ativos_na_classe = [
        ativo for ativo in portfolio_ativo
        if ativo['classe'] == classe_prioritaria
    ]

    universo_da_classe = [
        ativo for ativo in asset_universe
        if ativo['classe'] == classe_prioritaria and ativo.get('ativo', True)
    ]
    ...
    if len(ativos_na_classe) == 1:
        ...
        candidato = next(
            (a for a in universo_da_classe if a['ticker'] not in tickers_existentes),
            None
        )
    ...
    ativo_menor = min(
        ativos_na_classe,
        key=lambda x: x['quantidade'] * x.get('preco_atual', float('inf'))
    )
